...
const Settings: React.FC = () => {
    ...
    try {
      ...
      if (error instanceof AxiosError) {
        if (
          ...
          `Player '${currentUser.displayName}' not found.`
        ) {
          if (currentUser.uid) {
            await deleteUser(currentUser);
          }
    ...
    return (
      <div className="account-info space-y-6">
        ...
        <div className="space-y-2">
          <p>
            <strong>Player Name:</strong> {playerData.playername || "Not set"}
          </p>
          <p>
          ...
          >
            ...
            {isEditingAboutMe ? (
              <input
                ...
                min={1}
                ref={aboutMeRef}
                defaultValue={playerData.aboutme ?? ""}
                className="text-black border-2 border-blue-500 rounded-lg p-2"
              />
            ) : (
  ...
  const renderPlayerStats = () => {
    ...
    return (
      <div className="player-stats space-y-6">
        <div className="space-y-2">
          <p>
            <strong>Wins:</strong> {playerData.wins}
          </p>
          <p>
            <strong>Losses:</strong> {playerData.losses}
          </p>
          <p>
            <strong>Ties:</strong> {playerData.ties}
          </p>
          <p>
            <strong>Win/Loss Ratio:</strong> {playerData.wlratio}
          </p>
          <p>
            <strong>Tournament Wins:</strong> {playerData["tournament wins"]}
          </p>
          <p>
            <strong>Tournament Losses:</strong>{" "}
            {playerData["tournament losses"]}
          </p>
        </div>
  ...
  return (
    <div className="settings-page h-screen flex">
      <div
        className={`taskbar text-white p-6 flex flex-col items-center transition-all duration-300 ${
          isTaskbarExpanded ? "w-64" : "w-16"
        }`}
      ...
      >
        ...
        <button
          className={`mb-6 text-white hover:bg-[#F6B17A] hover:text-gray-300 transition-all duration-200 ${
            isTaskbarExpanded ? "w-full" : "w-16"
        ...
        <button
          className={`mt-auto text-white hover:bg-[#F6B17A] hover:text-gray-300 transition-all duration-200 ${
            isTaskbarExpanded ? "w-full" : "w-16"
          } h-12 flex items-center justify-center space-x-2`}
