...
const CampaignConfigurator = () => {
    ...
    triggers: [
      {
        ...
        parameters: [
          ...
          { name: "repeat", type: "select", label: "Repeat", options: ["None", "Daily", "Weekly", "Monthly", "Yearly"] }
        ]
      },
      {
        ...
        parameters: [
          { name: "from_status", type: "select", label: "From Status", options: ["Booked", "Checked In", "Completed", "Cancelled"] },
          { name: "to_status", type: "select", label: "To Status", options: ["Booked", "Checked In", "Completed", "Cancelled"] }
        ]
      }
    ],
    actions: [
  ...
  const renderField = ({ name, type, label, options }) => {
    ...
    return <Field type={type} name={name} className="block p-2 border rounded" />;
  };
                      ...
                      {config.triggers.map((trigger, index) => (
                          <div key={trigger.id}>
                            <label className="block cursor-pointer p-2 border rounded hover:bg-gray-100">
                              <Field type="radio" name="trigger" value={trigger.id} onClick={() => handleNextStep()} />
                ...
                {step === 1 && (
                    <div>
                      ...
                      {config.actions.map((action, index) => (
                          <div key={action.id}>
                            <label className="block cursor-pointer p-2 border rounded hover:bg-gray-100">
                              <Field type="radio" name="action" value={action.id} onClick={() => handleNextStep()} />
                            ...
                            <div>
                              ...
                              {config.conditions.map((condition, index) => (
                                  <div key={condition.id}>
                                    <h3 className="text-md font-bold mb-2">{condition.description}</h3>
                              ...
                              <button type="button" className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mt-4" onClick={() => arrayHelpers.push({})}>
                                Add Condition
                              </button>
                            </div>
                        )}
